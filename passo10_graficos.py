...
# ════════════════════════════════════════════════════════════════════════════
# GRÁFICO 4 — Valor total homologado por entidade por ano
# Requer coluna "nomeEntidade" no licitacoes_deflacionadas.csv
# ════════════════════════════════════════════════════════════════════════════
if "nomeEntidade" not in df.columns:
    print("⚠️  Gráfico 4 ignorado: coluna 'nomeEntidade' não encontrada.")
    print("   → Rode o passo2_consolidar.py atualizado e reprocesse a cadeia.")
else:
    df_hom = df[
        df["situacao"].str.upper().str.strip() == "HOMOLOGADO"
    ].drop_duplicates(subset=["ano", "objeto"]).copy()

    # Agrupa por entidade e ano, soma valorHomologado
    ent_ano = (
        df_hom.groupby(["ano", "nomeEntidade"])["valorHomologado"]
        .sum()
        .reset_index()
    )

    # Mantém só as top 6 entidades por valor total (evita legenda poluída)
    top_entidades = (
        ent_ano.groupby("nomeEntidade")["valorHomologado"]
        .sum()
        .nlargest(6)
        .index.tolist()
    )
    ...
    pivot = ent_ano_top.pivot_table(
        index="ano", columns="nomeEntidade", values="valorHomologado", aggfunc="sum"
    ).fillna(0)
    ...
    for i, col in enumerate(pivot.columns):
        ...
        # Abrevia nome longo para legenda
        label = col.replace("PREFEITURA MUNICIPAL DE ", "P.M. ") \
                   .replace("FUNDAÇÃO ", "FD. ") \
...
# Barras de valor total (eixo principal)
...
for i, mod in enumerate(top_modal):
    ...
    ax.plot(sub["ano"].astype(int), sub["indice_desconto_pct"],
            color=cores_modal[i], linewidth=2.2, marker="o", markersize=6,
            label=label, zorder=3)

ax.axhline(0, color="#aaaaaa", linewidth=0.8, linestyle=":")
ax.set_xlabel("Ano", fontsize=11)
ax.set_ylabel("Desconto mediano (%)", fontsize=11)
ax.set_title("Desconto Mediano por Modalidade ao Longo dos Anos\n(mediana por licitação, excluindo outliers)", pad=14)
...
# Requer coluna "tipoObjeto" no licitacoes_deflacionadas.csv
# ════════════════════════════════════════════════════════════════════════════
if "tipoObjeto" not in df.columns:
    ...
    # Normaliza categorias (algumas podem ter variações de texto)
    df_hom7["tipoObjeto"] = df_hom7["tipoObjeto"].str.strip().str.title()

    # Top 5 tipos por volume
    top_tipos = df_hom7["tipoObjeto"].value_counts().head(5).index.tolist()
    df_tipo = df_hom7[df_hom7["tipoObjeto"].isin(top_tipos)].copy()

    tipo_ano = (
        df_tipo.groupby(["ano", "tipoObjeto"])["valorHomologado"]
        .sum()
        .reset_index()
    )

    pivot_tipo = tipo_ano.pivot_table(
        index="ano", columns="tipoObjeto", values="valorHomologado", aggfunc="sum"
    ).fillna(0)
    ...
    for i, col in enumerate(pivot_tipo.columns):
        ...
        ax.bar(pivot_tipo.index.astype(int), pivot_tipo[col] / 1e6,
               bottom=bottom / 1e6, color=cores_tipo[i % len(cores_tipo)],
               alpha=0.88, width=0.6, label=label, zorder=3)
